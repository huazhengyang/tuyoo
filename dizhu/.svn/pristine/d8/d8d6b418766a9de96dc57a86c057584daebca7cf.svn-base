    ...
    def _getTable(self, player):
        ...
        for table in self._tableList:
            ...
            if table.state.name != 'idle':
                if ftlog.is_debug():
                    ftlog.debug('DizhuTableRoomSegmentMatchAI._getTable BadTableState',
                                'roomId=', self.roomId,
                                'userId=', player.userId,
    ...
    def getRoomOnlineInfo(self):
        ...
        for t in self.maptable.values():
            ...
            if players:
                ...
                ucount += 1
            if ftlog.is_debug():
                ftlog.debug('DizhuTableRoomSegmentMatchAI.getRoomOnlineInfo Single',
                            'roomId=', self.roomId, 'tableId=', t.tableId, 'players=', [p.userId for p in players])
        if ftlog.is_debug():
            ftlog.debug('DizhuTableRoomSegmentMatchAI.getRoomOnlineInfo Final',
                        'roomId=', self.roomId,
