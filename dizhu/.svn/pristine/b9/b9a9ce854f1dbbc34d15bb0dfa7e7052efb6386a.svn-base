...
from dizhu.entity import dizhuaccount, dizhumatchcond, dizhuhallinfo
...
from dizhu.erdayimatch.interfaceimpl import TableControllerErdayi, \
    PlayerNotifierErday, MatchRewardsErdayi, MatchUserIFErdayi, \
    SignerInfoLoaderErdayi, ErdayiMatchFactory, SigninFeeErdayi
...
from poker.entity.dao import sessiondata, userdata, gamedata, daobase
...
class ErdayiMatch(object):
    ...
    def checkMatchingSequence(cls, room, matchId):
        ...
        if not sequence:
            ...
            ftlog.info('ErdayiMatch.checkMatchingSequence',
                       'matchId=', matchId,
                       'key=', key,
    ...
    def getMatchStatesByPlayer(cls, match, room, player, mo):
        ...
        if ftlog.is_debug():
            ftlog.debug('ErdayiMatch.getMatchStatesByPlayer roomId=', room.bigRoomId,
    ...
    def doWinLose(cls, room, table, seatId, isTimeOutKill=False):
        ...
        for i, player in enumerate(table.players):
            ...
            # 计算本农民的倍数
            seatMulti = max(table.seats[i].seatMulti, 1) * max(table.seats[dizhuseatId - 1].seatMulti, 1)
            detalChip *= seatMulti
            if dizhuwin:
                detalChip *= -1
            ...
            if ftlog.is_debug():
                ftlog.debug('ErdayiMatch.doWinLoseTable roomId=', room.roomId,
                            'tableId=', table.tableId,
                            'seatId=', seatId,
                            'dizhuseatId=', dizhuseatId,
                            'detalChip=', detalChip,
                            'dizhuDetalChip=', detalChips[dizhuseatId - 1])
...
#         moWin.setResult('stat', dict(zip(tdz_stat_title, table.status)))
        ...
        for i, player in enumerate(table.players):
            ...
            mrank = 3
            mtableRanking = 3
            moWin.setResult('seat' + str(player.seatId), [detalChips[i], seat_coin[i], 0, 0, 0, 0, mrank, mtableRanking])
            if not player.isAI:
                #增加经验
                ...
                if ftlog.is_debug():
                    ftlog.debug('ErdayiMatch.doWinLoseTable',
                                'addExp=', 20,
                                'curExp=', exp,
                                'curLevel=', explevel)
    ...
    def _handleSigninFeeNotEnoughException_V3_772(cls, room, ex, uid, mo):
        payOrder = ex.fee.getParam('payOrder')
        clientId = sessiondata.getClientId(uid)
        ftlog.debug("groupmatch._handleSigninFeeNotEnoughException_V3_772", "userId", uid,
                    "fee.itemId=", ex.fee.assetKindId,
                    "fee.params=", ex.fee.params,
                    "payOrder=", payOrder)
