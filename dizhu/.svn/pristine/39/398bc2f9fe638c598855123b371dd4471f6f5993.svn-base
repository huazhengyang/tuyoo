...
from poker.entity.game.rooms.erdayi_match_ctrl.models import Player, PlayerSort, \
    PlayerQueuing, Signer
...
class ErdayiPlayer(Player):
    def __init__(self, userId):
        ...
        self.playTime = 0
        self.waitReason = WaitReason.WAIT
    ...
    def buildTableInfoMessage(cls, table, msg):
        ...
        for seat in table.seats:
            ...
            if not seat.player:
                ...
                seatInfo['isQuit'] = seat.player.isQuit
...
class PlayerNotifierErday(PlayerNotifier):
    ...
    def notifyMatchOver(self, player, reason, rankRewards):
        ...
        try:
            ...
            try:
                ...
                # 设置奖状分享大图
                rankRewardsList = self._room.roomConf.get('matchConf', {}).get('rank.rewards', [])
    ...
    def notifyMatchRank(self, player):
        ...
        ranktops.append({'userId':player.userId,
                         ...
                         'score':erdayi3api.fmtScore(player.score),
                         'rank':str(player.rank),
                         'curScore':erdayi3api.fmtScore(lastCardResult.mpRatioScore if lastCardResult else 0)})
        for rp in player.stage.rankList:
            lastCardResult = rp.getLastCardResult()
            ranktops.append({'userId':rp.userId,
                             'name':rp.userName,
                             'score':erdayi3api.fmtScore(rp.score),
                             'rank':str(rp.rank),
                             'curScore':erdayi3api.fmtScore(lastCardResult.mpRatioScore if lastCardResult else 0)})
            msg.setResult('mranks', ranktops)
            if self._logger.isDebug():
                self._logger.debug('detail PlayerNotifierDizhu.notifyMatchRank',
                                   'rp.userId=', rp.userId,
                                   'name=', rp.userName,
                                   'rp.score=', rp.score,
                                   'score=', erdayi3api.fmtScore(rp.score),
...
class ErdayiStage(MatchStage):
    ...
    def _doPlayerMatchOver(self, player, reason):
        ...
        self._logger.info('ErdayiStage._doPlayerMatchOver',
                          'userId=', player.userId,
